...
import (
	...
	"github.com/LLKennedy/go-card/smartcard/pcsc"
)
...
// WaitForChanges starts waiting for any changes and reporting them back on the created channel
func (ctx *Context) WaitForChanges(closeChan <-chan struct{}) (<-chan []pcsc.ReaderState, <-chan error) {
	out := make(chan []pcsc.ReaderState)
